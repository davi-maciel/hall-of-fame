#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/pagmo.json.
...
Raw record shape (data/raw/pagmo.json), a JSON list of:
  {"rawName": str, "olympiadId": "pagmo", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/pagmo.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "pagmo")
        medal = r.get("medal")
