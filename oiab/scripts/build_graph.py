#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/oiab.json.
...
Raw record shape (data/raw/oiab.json), a JSON list of:
  {"rawName": str, "olympiadId": "oiab", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/oiab.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "oiab")
        medal = r.get("medal")
