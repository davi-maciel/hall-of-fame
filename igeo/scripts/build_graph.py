#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/igeo.json.
...
Raw record shape (data/raw/igeo.json), a JSON list of:
  {"rawName": str, "olympiadId": "igeo", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/igeo.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "igeo")
        medal = r.get("medal")
