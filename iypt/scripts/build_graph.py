#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/iypt.json.
...
Raw record shape (data/raw/iypt.json), a JSON list of:
  {"rawName": str, "olympiadId": "iypt", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/iypt.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "iypt")
        medal = r.get("medal")
