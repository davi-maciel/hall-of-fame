#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/imcho.json.
...
Raw record shape (data/raw/imcho.json), a JSON list of:
  {"rawName": str, "olympiadId": "imcho", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/imcho.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "imcho")
        medal = r.get("medal")
