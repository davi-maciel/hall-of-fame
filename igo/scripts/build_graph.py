#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/igo.json.
...
Raw record shape (data/raw/igo.json), a JSON list of:
  {"rawName": str, "olympiadId": "igo", "year": int,
   "medal": "gold"|"silver"|"bronze"|"honorable-mention"|null,
   "level": "elementar"|"intermediario"|"avancado", "score": int (optional)}
"""
import json
import re
import sys
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/igo.json first")
