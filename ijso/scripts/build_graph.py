#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/ijso.json.
...
Raw record shape (data/raw/ijso.json), a JSON list of:
  {"rawName": str, "olympiadId": "ijso", "year": int,
...
def make_slug(name: str) -> str:
    ...
    base = base.replace("'", "").replace("\u2019", "")  # repo rule for new folders: drop apostrophes
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/ijso.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "ijso")
        medal = r.get("medal")
