#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/oiaq.json.
...
Raw record shape (data/raw/oiaq.json), a JSON list of:
  {"rawName": str, "olympiadId": "oiaq", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/oiaq.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "oiaq")
        medal = r.get("medal")
