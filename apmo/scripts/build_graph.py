#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/apmo.json.
...
Raw record shape (data/raw/apmo.json), a JSON list of:
  {"rawName": str, "olympiadId": "apmo", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/apmo.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "apmo")
        medal = r.get("medal")
