#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/rioplatense.json.
...
Raw record shape (data/raw/rioplatense.json), a JSON list of:
  {"rawName": str, "olympiadId": "rioplatense", "year": int,
   "medal": "gold"|"silver"|"bronze"|"honorable-mention"|null,
   "level": "A"|"1"|"2"|"3", "team": "Fortaleza"|"São Paulo"|"Brasil"}
"""
import json
import re
import sys
...
RAW = ROOT / "data" / "raw" / "rioplatense.json"
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/rioplatense.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "rioplatense")
        medal = r.get("medal")
