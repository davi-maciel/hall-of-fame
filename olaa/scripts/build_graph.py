#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/olaa.json.
...
Raw record shape (data/raw/olaa.json), a JSON list of:
  {"rawName": str, "olympiadId": "olaa", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/olaa.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "olaa")
        medal = r.get("medal")
