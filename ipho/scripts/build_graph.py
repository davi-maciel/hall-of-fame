#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/ipho.json.
...
Raw record shape (data/raw/ipho.json), a JSON list of:
  {"rawName": str, "olympiadId": "ipho", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/ipho.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "ipho")
        medal = r.get("medal")
