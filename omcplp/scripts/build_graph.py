#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/omcplp.json.
...
Raw record shape (data/raw/omcplp.json), a JSON list of:
  {"rawName": str, "olympiadId": "omcplp", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/omcplp.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "omcplp")
        medal = r.get("medal")
