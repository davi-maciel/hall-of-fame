#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/wopho.json.
...
Raw record shape (data/raw/wopho.json), a JSON list of:
  {"rawName": str, "olympiadId": "wopho", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/wopho.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "wopho")
        medal = r.get("medal")
