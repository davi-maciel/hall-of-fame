#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/ioaa.json.
...
Raw record shape (data/raw/ioaa.json), a JSON list of:
  {"rawName": str, "olympiadId": "ioaa", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/ioaa.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "ioaa")
        medal = r.get("medal")
