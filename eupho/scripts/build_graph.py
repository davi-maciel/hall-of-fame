#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/eupho.json.
...
Raw record shape (data/raw/eupho.json), a JSON list of:
  {"rawName": str, "olympiadId": "eupho", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/eupho.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "eupho")
        medal = r.get("medal")
