#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/apho.json.
...
Raw record shape (data/raw/apho.json), a JSON list of:
  {"rawName": str, "olympiadId": "apho", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/apho.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "apho")
        medal = r.get("medal")
