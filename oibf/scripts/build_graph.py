#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/oibf.json.
...
Raw record shape (data/raw/oibf.json), a JSON list of:
  {"rawName": str, "olympiadId": "oibf", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/oibf.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "oibf")
        medal = r.get("medal")
