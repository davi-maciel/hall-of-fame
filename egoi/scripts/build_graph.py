#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/egoi.json.
...
Raw record shape (data/raw/egoi.json), a JSON list of:
  {"rawName": str, "olympiadId": "egoi", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/egoi.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "egoi")
        medal = r.get("medal")
