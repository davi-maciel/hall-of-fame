#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/egmo.json.
...
Raw record shape (data/raw/egmo.json), a JSON list of:
  {"rawName": str, "olympiadId": "egmo", "year": int,
   "medal": "gold"|"silver"|"bronze"|"honorable-mention"|null,
   "score": int (optional, total points)}
"""
import json
import re
import sys
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/egmo.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "egmo")
        medal = r.get("medal")
