#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/rmm.json.
...
Raw record shape (data/raw/rmm.json), a JSON list of:
  {"rawName": str, "olympiadId": "rmm", "year": int,
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/rmm.json first")
