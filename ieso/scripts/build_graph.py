#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/ieso.json.
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/ieso.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "ieso")
        medal = r.get("medal")
