#!/usr/bin/env python3
"""Build src/data/graph.json from data/raw/nbpho.json.
...
Raw record shape (data/raw/nbpho.json), a JSON list of:
  {"rawName": str, "olympiadId": "nbpho", "year": int,
...
def make_slug(name: str) -> str:
    ...
    base = base.replace("'", "").replace("\u2019", "")  # small-world drops apostrophes
...
def main():
    if not RAW.exists():
        sys.exit(f"missing {RAW}; create data/raw/nbpho.json first")
    ...
    for r in records:
        raw = r["rawName"]
        year = int(r["year"])
        olympiad = r.get("olympiadId", "nbpho")
        medal = r.get("medal")
